...
centroids_path = strcat('debug_centroids_sample_', num2str(N),'_',color_space,'.mat');
vocabulary_size = 400;
fast_kmeans = false;
...
if exist(classes_path, 'file')
    ...
    for current_class = 1:length(class_names)
        image_files = dir(strcat(image_data_path, class_names{current_class}, '_train/*jpg'));
        image_files = {image_files.name};
        %         image_files(1:2) = []; % remove first junk files
        
        image_files_test = dir(strcat(image_data_path, class_names{current_class}, '_test/*jpg'));
    ...
    for current_class = 1:length(classes)
        ...
        for i = 1:length(classes(current_class).image_files_test)
            ...
            %             classes(current_class).images_test(i).d_RGB_dense = d_RGB_dense;
            classes(current_class).images_test(i).d_rgb = d_rgb;
            %             classes(current_class).images_test(i).d_rgb_dense = d_rgb_dense;
            classes(current_class).images_test(i).d_gray = d_gray;
            classes(current_class).images_test(i).d_gray_dense = d_gray_dense;
            classes(current_class).images_test(i).d_opponent = d_opponent;
            %             classes(current_class).images_test(i).d_opponent_dense = d_opponent_dense;
        end
    end
...
if exist('debug_svm_models.mat', 'file')
    load('debug_svm_models.mat')
else
    % Train models
...
for eval_class = 1:length(classes)
    hists_to_predict = array2table(zeros(length(classes(current_class).images_test)* ...
        length(classes), vocabulary_size)); % +1 for im_path
%     hists_to_predict.Properties.VariableNames(vocabulary_size+1) = {'class'};
    %     hists_to_predict.Properties.VariableNames(vocabulary_size+2) = {'im_path'};
    d_index = 1;
    for current_class = 1:length(classes)
        for i = 1:length(classes(current_class).images_test)  % images
            image_descriptors = cell([1 length(spaces)*3]);
            channels = fieldnames(classes(current_class).images_test(i).(color_space));
            d=1;
            for k = 1:length(channels)
                descriptor = ...
                    classes(current_class).images_test(i).(color_space).(channels{k});
                image_descriptors{d} = descriptor;
                d = d+1;
            end
            image_descriptors = image_descriptors(~cellfun('isempty', image_descriptors));
            warning('off','all');
            hists_to_predict(d_index,1:vocabulary_size) = array2table(getHistFromDescriptor(double(cell2mat(image_descriptors)'), centroids));
            warning('on','all');
            d_index = d_index+1;
%             hists_to_predict(d_index,vocabulary_size+1) = {current_class};
            %             hists_to_predict(d_index,vocabulary_size+2) = classes(current_class).images_test(i).im_path;
        end
    end
    [label,Score] = predict(classes(eval_class).svm_model, hists_to_predict);
end

save('debug_all_files_after_eval');
